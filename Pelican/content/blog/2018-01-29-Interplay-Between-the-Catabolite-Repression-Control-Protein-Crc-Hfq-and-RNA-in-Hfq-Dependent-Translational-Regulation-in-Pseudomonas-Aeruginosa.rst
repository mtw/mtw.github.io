...
Seen from the perspective of the later posts on this site, this paper provides the missing molecular explanation for several phenotypes that otherwise look disconnected. The 2018 metabolic sensitization paper uses `CrcZ` to shift Hfq-dependent antibiotic susceptibility. The 2020 carbapenem paper shows that `opdP` is controlled through an `Hfq/Crc` complex while `oprD` follows a different Hfq/sRNA route. And the 2022 diauxic-growth study expands that logic to broader transcriptome rewiring. This 2018 NAR paper is the place where the physical and biochemical basis of that regulatory architecture is laid out.
