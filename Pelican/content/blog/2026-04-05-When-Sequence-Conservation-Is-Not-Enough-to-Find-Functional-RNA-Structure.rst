...
That is why comparative RNA analysis has to ask a different question. Instead of looking only for segments that stay similar letter by letter, it asks whether the same architectural idea has been preserved. Are there stems that remain paired through compensatory substitutions. Do different lineages reuse the same motif arrangement even when local sequence detail changes. Does a conserved mechanistic role survive despite obvious sequence divergence.
...
This matters because functional RNA elements are not always portable as simple motifs. An xrRNA is not defined only by a short consensus sequence. It is defined by a fold that creates a physical barrier to exonuclease progression. The same is true more broadly for many structured elements in viral genomes. Their biological activity depends on geometry, context, and interaction logic. Those are precisely the features that sequence identity alone captures poorly.
...
This is why I think structure conservation is one of the more important conceptual shifts in RNA virology. It changes what counts as homology, what counts as divergence, and what kinds of questions we can ask about function. Instead of treating sequence divergence as evidence that structure has been lost, we can ask whether the function has simply been re-encoded in a different sequence realization.

The practical consequence is straightforward. When the biology depends on structured RNA, sequence alignment should usually be the beginning of the analysis, not the end. Functional interpretation gets much stronger once covariation, consensus folding, experimental validation, and comparative architecture are brought into the same picture.
