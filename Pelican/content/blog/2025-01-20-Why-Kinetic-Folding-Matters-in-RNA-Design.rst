...
That is why kinetic folding matters. The real issue is not whether equilibrium thermodynamics remains valuable, because it does. The issue is whether the design objective depends only on the endpoint or on the route taken to get there. Once the route matters, sequence ranking based on static structure alone starts to miss the real failure modes.
